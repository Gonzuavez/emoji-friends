...
Use one canonical identity **`bruno-en-v1`**, English: youthful/childlike (roughly 8–10-year-old character feel), light, warm, cheerful, curious, playful, gentle, expressive, natural, family-friendly. Never squeaky/shrill, deep/adult, announcer-like, or corporate. This identity and direction live separately from gift text and clip URLs, ready for future occasions. Record the approved scripts with the same approved voice; do not independently choose a new voice for each gift.

Future reuse should match `(voice identity, language, exact script)`; changing a recipient, sender, or message requires a matching clip or caption-only fallback. These demo files must not be reused for different names/text. No voice generation, provider SDK, API keys, or paid TTS calls run in the client. Any future generation workflow needs a separate work order.
